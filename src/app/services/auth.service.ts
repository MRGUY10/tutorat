...
export class AuthService {
    ...
    startDemoSession(role: UserRole = UserRole.ADMIN): void {
        const now = Math.floor(Date.now() / 1000);
        const exp = now + (24 * 60 * 60);
        const userType: UserType = role as unknown as UserType;

        const header = { alg: 'HS256', typ: 'JWT' };
        const payload = {
            sub: `demo-${role.toLowerCase()}@edututor.dev`,
            email: `demo.${role.toLowerCase()}@edututor.dev`,
            role,
            userId: role === UserRole.ADMIN ? 1 : role === UserRole.TUTOR ? 2 : 3,
            userType: role,
            iat: now,
            exp
        };
        ...
        const token = `${encode(header)}.${encode(payload)}.demo-signature`;

        const authResponse: AuthResponse = {
            token,
            refreshToken: `demo-refresh-token-${role.toLowerCase()}`,
            expiresIn: 24 * 60 * 60,
            email: payload.email,
            role,
            ...
            userInfo: {
                id: payload.userId,
                nom: role === UserRole.ADMIN ? 'Admin' : role === UserRole.TUTOR ? 'Tutor' : 'Student',
                prenom: 'Demo',
                email: payload.email,
                roles: [role],
                userType: userType
        ...
        const userInfo: CurrentUser = {
            id: payload.userId,
            nom: authResponse.userInfo?.nom || 'Demo',
            prenom: authResponse.userInfo?.prenom || 'User',
            email: payload.email,
            role,
            userType: userType,
            roles: [role],
        ...
        this.currentUserInfoSubject.next(userInfo);
        this.updateLoginState(authResponse, userInfo);
    }
