...
} from '../core/models/tutor-backend.model';
...
export class TutorService {
  ...
  /**
   * Unverify tutor (Admin only)
   * PUT /api/tutors/{id}/unverify
   */
  unverifyTutor(id: number): Observable<TutorProfileResponse> {
    return this.makeRequest<TutorProfileResponse>('PUT', `${this.baseApiUrl}/${id}/unverify`, {})
      .pipe(tap(() => this.refreshTutors()));
  }
  ...
  private convertSummaryToCard(summary: TutorSummaryResponse): TutorCard {
    return {
      id: summary.id,
      name: summary.fullName || 'Unknown',
      rating: summary.noteMoyenne || 0,
      reviewCount: summary.nombreEvaluations || 0,
      tarif: summary.tarifHoraire,
      location: `${summary.ville || ''}${summary.ville && summary.pays ? ', ' : ''}${summary.pays || ''}`,
      isVerified: summary.verifie || false,
      isOnline: summary.disponible || false,
